#!/usr/bin/env bash
# Daily auto-sync: pull upstream README → re-parse → push if changed
...
# Use Homebrew binaries explicitly (launchd has minimal PATH)
...
# Pull any remote changes first (avoid push conflicts)
...
# Fetch fresh upstream data and re-parse
npm run --silent fetch-source >>"$LOG_FILE" 2>&1
...
# No changes? Quietly exit (no notification noise).
...
log "Changes detected: $OLD_COUNT → $NEW_COUNT (delta=$DELTA)"

# Commit and push
git add src/data/prompts.json src/data/meta.json
git -c user.email=takesuzue8282@gmail.com -c user.name=Pokare \
    commit -q -m "Auto-sync: $NEW_COUNT prompts (Δ$DELTA)"
